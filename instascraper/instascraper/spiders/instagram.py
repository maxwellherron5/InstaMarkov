...
class InstagramSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for username in user_accounts:
            ...
            yield scrapy.Request(self.get_url(url), callback=self.parse)
    ...
    def parse(self, response):
        ...
        for i in edges:
            ...
            item = {'captions': captions[:-1]}
            if video:
                yield scrapy.Request(self.get_url(url), callback=self.get_video, meta={'item': item})
            else:
                item['videoURL'] = ''
                yield item
        if next_page_bool:
            ...
            yield scrapy.Request(self.get_url(url), callback=self.parse_pages, meta={'pages_di': di})
